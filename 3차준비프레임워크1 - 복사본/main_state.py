...
class Balance_Enemy3: # Slime
    ...
    def __init__(self):
        if Balance_Enemy3.image == None:
            Balance_Enemy3.image = load_image('./resource/예티.png')
        if Balance_Enemy3.HP_image == None:
            Balance_Enemy3.HP_image = load_image('./resource/HP.png')
        #
        #if Balance_Enemy2.makesound  == None:
        #    Balance_Enemy2.makesound = load_wav('./resource/발록등장.wav')
        if Balance_Enemy3.attacksound  == None:
            Balance_Enemy3.attacksound = load_wav('./resource/예티공격.wav')
    ...
    def Attack(self):
        self.attackstate=1
        if ( self.frame != 0 ) and ( self.Timer % 5 == 0 ):
            Balance_Enemy3.attacksound.play()
            self.frame = (self.frame+1) % 5
            Barricade.HP -= self.ATK
            if(stage==1):
                Barricade.HP1 -= self.ATK
    ...
    def Die(self):
        ...
        if self.frame == 30:
            ...
            #Balance_Enemy3.winsound.play()
            #4self.winimage.draw(600,300)
            self.frame += 1
        self.Timer+=1
    ...
    def draw(self):
        if(self.attackstate!=1):
            self.image.clip_draw(self.frame * 120, 920, 120, 120, self.x, self.y)
        if(self.attackstate==1):
            self.image.clip_draw((self.frame+1) * 137, 700, 140, 200, self.x-5, self.y+30)
        self.HP_image.clip_draw( (round( (self.HP / self.First_HP * 5) ) ) * 48, 0, 48, 6, self.x, self.y+50)
...
class Balance_Enemy4: # Slime
    ...
    def __init__(self):
        if Balance_Enemy4.image == None:
            Balance_Enemy4.image = load_image('./resource/상어.png')
        if Balance_Enemy4.HP_image == None:
            Balance_Enemy4.HP_image = load_image('./resource/HP.png')
        #
        #if Balance_Enemy2.makesound  == None:
        #    Balance_Enemy2.makesound = load_wav('./resource/발록등장.wav')
        if Balance_Enemy4.attacksound  == None:
            Balance_Enemy4.attacksound = load_wav('./resource/상어공격.wav')
    ...
    def Attack(self):
        self.attackstate=1
        if ( self.frame != 0 ) and ( self.Timer % 5 == 0 ):
            Balance_Enemy4.attacksound.play()
            self.frame = (self.frame+1) % 5
            Barricade.HP -= self.ATK
            if(stage==1):
                Barricade.HP1 -= self.ATK
    ...
    def draw(self):
        if(self.attackstate!=1):
            self.image.clip_draw(self.frame * 140, 910, 140, 120, self.x, self.y)

        if(self.attackstate==1):
            self.image.clip_draw(self.frame * 160, 360, 140, 120, self.x, self.y)
        self.HP_image.clip_draw( (round( (self.HP / self.First_HP * 5) ) ) * 48, 0, 48, 6, self.x, self.y+50)

    def __del__(self):
        pass

class Balance_Enemy5: # Slime
    ...
    def __init__(self):
        if Balance_Enemy5.image == None:
            Balance_Enemy5.image = load_image('./resource/용.png')
        if Balance_Enemy5.HP_image == None:
            Balance_Enemy5.HP_image = load_image('./resource/HP.png')
        #
        #if Balance_Enemy2.makesound  == None:
        #    Balance_Enemy2.makesound = load_wav('./resource/발록등장.wav')
        if Balance_Enemy5.attacksound  == None:
            Balance_Enemy5.attacksound = load_wav('./resource/용공격.wav')
    ...
    def Move(self):
        ...
        if self.Timer == 20:
            self.x = self.x-40
            self.Timer = 0
            if self.x-500 <= (Barricade.Wallx - ( (Barricade.Wally - self.y) / 5.5 ) ):
                self.frame = 0
                self.state = self.ATTACK
                self.Timer = 0
    ...
    def Attack(self):
        self.attackstate=1
        if ( self.frame != 0 ) and ( self.Timer % 5 == 0 ):
            Balance_Enemy5.attacksound.play()
            self.frame = (self.frame+1) % 5
            Barricade.HP -= self.ATK
            if(stage==1):
                Barricade.HP1 -= self.ATK
    ...
    def draw(self):
        if(self.attackstate!=1):
            self.image.clip_draw(self.frame * 230-(6*self.frame), 2400, 200, 230, self.x, self.y)


        if(self.attackstate==1):
            self.image.clip_draw(self.frame * 260-(5*self.frame)-30, 1450, 250, 320, self.x, self.y)
        self.HP_image.clip_draw( (round( (self.HP / self.First_HP * 5) ) ) * 48, 0, 48, 6, self.x, self.y+50)
